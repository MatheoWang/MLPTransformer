...
class Early_stopping():
    ...
    def __call__(self, cur_loss, cur_epoch):
        best_loss = False
        self.pass_loss.append(cur_loss)
        if len(self.pass_loss) > 5:
            val_loss = np.mean(self.pass_loss[-5:])
        else:
            val_loss = cur_loss
...
def generate_attn_mask(n_size, num_patchx):
    num_patch = num_patchx**2
    ori = np.ones((num_patch, num_patchx, num_patchx))
    for i in range(num_patch):
        row = i//num_patchx
        col = i%num_patchx
        idx = [max(0, col-n_size), min(num_patchx, col + n_size + 1)]
        idy = [max(0, row-n_size), min(num_patchx, row + n_size + 1)]
        ori[i, idy[0]:idy[1], idx[0]:idx[1]] = 0
    ori_mask = ori.reshape((num_patch, num_patch))
    return ori_mask
...
def parse_config(config_path):
    ...
    args = {}
    for key, sec in config.items():
        for _key, _value in sec.items():
            # int
            if _key == 'batch_size' or _key == 'epochs' or _key == 'steps_per_epoch' or \
                    _key == 'initial_epoch' or _key == 'int_steps' or _key == 'int_downsize' or\
                    _key =='test_fold' or _key == 'img_size' or _key == 'src_feats':
                args[_key] = int(sec[_key])
            # float
            elif _key == 'lr' or _key == 'deform_lambda' or _key == 'deform_lambda2' or _key == 'dropout' \
                    or _key == 'affine_lambda':
                args[_key] = float(sec[_key])
            # boolean
            elif _key == 'cudnn_det' or _key == 'bidir' or _key=='multichannel' or _key == 'validation' or\
                    _key == 'rand_pair' or _key == 'load_pretrain' or _key == 'mlp' or _key == 'lr_step' or\
                    _key == 'img_crop' or _key == 'learn_pos' or _key=='single_scale' or _key == 'cnnout' or\
                    _key == 'int_first':
                args[_key] = sec.getboolean(_key)
            # list
            elif _key == 'enc' or _key == 'dec' or _key == 'gpu' or _key == 'patch_size' or \
                    _key == 'out_dim' or _key=='dim' or _key=='depth' or _key=='heads' or _key == 'dim_forward' or \
                    _key == 'neigh_size' or _key=='window_size':
                args[_key] = [int(ch) for ch in sec[_key].split(',')]
            else:
                args[_key] = sec[_key]
    return args


def generate_random_pairs(array_idx, array_frame, rand_pair = False):
    if rand_pair is False:
        files_2ch = np.array([array_idx, np.array([2] * len(array_idx))]).T
        files_4ch = np.array([array_idx, np.array([4] * len(array_idx))]).T
    else:
        files_2ch = []
        files_4ch = []
        for i in range(len(array_idx)):
            ...
            for p in range(num_pair):
                rand2ch = np.random.randint(array_frame[i, 0], 2)
                files_2ch.append(np.array([array_idx[i], 2, rand2ch[0], rand2ch[1]]))
                rand4ch = np.random.randint(array_frame[i, 1], 2)
                files_4ch.append(np.array([array_idx[i], 4, rand4ch[0], rand4ch[1]]))
            '''
            fb_2ch = array_frame[i, 0]
            fb_4ch = array_frame[i, 1]
            cur_2ch = np.array([np.array([array_idx[i]] * fb_2ch),
                                       np.array([2] * fb_2ch),
                                       np.array([0] * fb_2ch),
                                       np.arange(0, fb_2ch)]).T
            files_2ch.append(cur_2ch)
            cur_4ch = np.array([np.array([array_idx[i]] * fb_4ch),
                                       np.array([4] * fb_4ch),
                                       np.array([0] * fb_4ch),
                                       np.arange(0, fb_4ch)]).T
            files_4ch.append(cur_4ch)
        files_2ch = np.concatenate(files_2ch, axis=0)
        files_4ch = np.concatenate(files_4ch, axis=0)
    return files_2ch, files_4ch


def get_train_valid_test_allfiles(valid_fold, folder_prefix, ch='2ch', generate_rand_pair=False, delete_poor=True):
    all_df = pd.read_csv(os.path.join(folder_prefix, 'all_info.csv'))
    # test fold: validation fold
    if delete_poor:
        all_df = all_df[all_df['ImageQuality_2ch'] != 'Poor'][all_df['ImageQuality_4ch'] != 'Poor']
    test_fold = [(valid_fold + 1) % 10, (valid_fold + 2) %10]
    train_fold = set(np.arange(1, 11)).symmetric_difference(set([test_fold[0], test_fold[1], valid_fold]))

    # test_files
    test_df = all_df[all_df.Fold.isin(test_fold)]
    test_num = test_df['Number'].values
    test_nb_frame = test_df[['NbFrame_2ch', 'NbFrame_4ch']].values
    test_files_2ch, test_files_4ch = generate_random_pairs(test_num, test_nb_frame, rand_pair=generate_rand_pair)

    # valid files
    valid_df = all_df[all_df['Fold'] == valid_fold]
    valid_num = valid_df['Number'].values
    valid_nb_frame = valid_df[['NbFrame_2ch', 'NbFrame_4ch']].values
    valid_files_2ch, valid_files_4ch = generate_random_pairs(valid_num, valid_nb_frame, rand_pair=generate_rand_pair)

    # train_files
    train_df = all_df[all_df.Fold.isin(train_fold)]
    train_num = train_df['Number'].values
    train_nb_frame = train_df[['NbFrame_2ch', 'NbFrame_4ch']].values
    train_files_2ch, train_files_4ch = generate_random_pairs(train_num, train_nb_frame, rand_pair=generate_rand_pair)
...
def load_volfile(
    ...
    newsize = [afterresize_shape for _ in vol.shape]

    # resizing
    if np_var != 'seg':
        vol = resize(vol, newsize, anti_aliasing=True)
    else:
        vol = np.round(resize(vol.astype(float), newsize, order=0)).astype(int)
...
def resize_by_factor(array, factor, batch_axis=False):
    """
